...
def input_data(result):
    try:
        ...
        # 将当页的内容一条条插入进数据库
        for j in result:
            sql = "insert into fund_010391(fund_id,fund_date,fund_nav,fund_accnav,fund_dgr,subsribe_state,redeem_state) values ('%s','%s','%s','%s','%s','%s','%s');"
...
def clear_data():
    try:
        cursor = connection.cursor()
        connection.begin()
        cursor.execute("truncate table fund_010391")
        connection.commit()
        print("清空数据表fund_010391")
    except Exception as input_exception:
        connection.rollback()
        connection.close()
        print(input_exception)
...
def update_fund_list():
    try:
        ...
        connection.begin()
        # 查询最新的记录
        sql = "select fund_id,fund_nav,fund_accnav,fund_date,fund_dgr,subsribe_state,redeem_state from fund_010391 order by fund_date desc limit 1"
        cursor.execute(sql)
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(e)
    # 创建连接获取源码
    response = create_request("010391", 1, 20, "2012-01-01", "2040-02-04")
    ...
    for i in range(1, int(max_page) + 1):
        ...
        response = create_request("010391", i, 20, "2012-01-01", "2040-02-04")
        response.encoding = "utf-8"
        # 解析获取当页结果
